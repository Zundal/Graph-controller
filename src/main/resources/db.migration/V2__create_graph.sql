...
CREATE INDEX idx_closure_ancestor ON user_group_closure (ancestor_group_id) COMMENT '상위 그룹 ID를 기준으로 빠른 검색을 위한 인덱스';
CREATE INDEX idx_closure_descendant ON user_group_closure (descendant_group_id) COMMENT '하위 그룹 ID를 기준으로 빠른 검색을 위한 인덱스';
